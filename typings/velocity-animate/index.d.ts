...
interface JQuery {
	velocity(name: string, options: jquery.velocity.RegisteredEffectOptions | boolean): JQuery;
...
declare namespace jquery.velocity {
	...
	interface VelocityStatic {
		(node: HTMLElement, action: string, queue?: string): Promise<void>
		(node: HTMLElement, properties: Properties, options: Options)
		...
		Utilities: {
			dequeue(node: HTMLElement, queue: string): void;
		}
	}
}